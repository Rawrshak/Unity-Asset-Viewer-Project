...
public class Static3dAssetManager : MonoBehaviour
{
    ...
    public async Task LoadStaticObjectAssets(List<KeyValuePair<Asset, int>> staticObjectAssets)
    {
        ...
        foreach (var pair in staticObjectAssets)
        {
            ...
            data.text = IsStatic3DAssetSupported(pair.Key) ? pair.Key.assetName : "[StandaloneWindows Unsupported] " + pair.Key.assetName;
            m_assets.Add(pair);
    ...
    async Task StaticObjectSelectedValueChanged()
    {
        if (m_staticObjectSelectorDropdown.value < m_assets.Count)
        {
            ...
            if (prefab == null) {
                m_staticObjectSelectorDropdown.options[m_staticObjectSelectorDropdown.value].text = m_staticObjectSelectorDropdown.options[m_staticObjectSelectorDropdown.value].text + " [StandaloneWindows Unsupported]";
    ...
    bool IsStatic3DAssetSupported(Asset asset)
    {
        ...
        if (!static3dObjectAsset.IsPlatformSupported(Static3dObjectAssetBase.Platform.StandaloneWindows) ||
